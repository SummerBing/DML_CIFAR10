...
class PyramidNet_feature(nn.Module):
    def __init__(self, depth, alpha, bottleneck=True):
        super(PyramidNet_feature, self).__init__()
        self.inplanes = 16
        if bottleneck == True:
            n = int((depth - 2) / 9)
            block = Bottleneck
...
class PyramidNet_classifier(nn.Module):
    def __init__(self, depth, alpha, num_classes, bottleneck=True):
        super(PyramidNet_classifier, self).__init__()
        self.inplanes = 16
        if bottleneck == True:
            n = int((depth - 2) / 9)
            block = Bottleneck
