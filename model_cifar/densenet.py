...
class DenseNet(nn.Module):
    def __init__(self, growth_rate=12, block_config=[16, 16, 16], compression=0.5,
                 ...
                 num_class=100, small_inputs=True, efficient=False):
...
class DenseNet_classifier(nn.Module):
    def __init__(self, num_class=100):
        ...
        # Linear layer
        self.classifier = nn.Linear(132, num_class)
...
class DenseNet_DML(nn.Module):
    def __init__(self, num_class, setup):
        super(DenseNet_DML, self).__init__()
        if setup == '40k12':
            self.growth_rate = 12
            self.block_config = [6,6,6]
        elif setup == '100k12':
            self.growth_rate = 12
            self.block_config = [16,16,16]
        elif setup == '190k12':
            self.growth_rate = 12
            self.block_config = [31,31,31]
        elif setup == '100k40':
            self.growth_rate = 40
            self.block_config = [16,16,16]
        else:
            print('Wrong network setup!')
        self.feature_extractor = DenseNet_feature(growth_rate=self.growth_rate, block_config=self.block_config)
        self.clf1 = DenseNet_classifier(num_class=num_class)
        self.clf2 = DenseNet_classifier(num_class=num_class)
...
if __name__ == '__main__':
    ...
    net = DenseNet_DML(num_class=100,growth_rate=12, block_config=[6,6,6])
    o1, o2 = net(ii)
    print(o1.size())